...
なお，序章で述べたように，このLogit選択確率は摂動効用理論の観点からも導出できる．
具体的には，需要ベクトル $\bm{p}$（各選択肢の選択確率）に対して，期待効用とShannon entropy正則化項からなる以下の直接効用関数
\begin{equation}\label{eq:perturbed_utility}
  \max_{\bm{p} \geq \bm{0},\, \sum_a p_a = 1} \left\{ \sum_a p_a V_a + \frac{1}{\mu}\mathcal{H}(\bm{p}) \right\}, \quad \mathcal{H}(\bm{p}) = -\sum_a p_a \ln p_a
\end{equation}
